...
| --- | --- | --- | --- |
| 40 decks x 100 runs | ~146 s | > 1 h (killed) | infeasible |
| 8 decks x 50 runs | 9.9 s | 655 s (~66x) | hours / explodes |
...
- v1 optimizes ranking accuracy (a proxy). Keep pairwise ranking but:
  ...
  - select the final model by **realized-EV gap** (section Phase 0 metric), not ndcg/
    regret alone.
...
- Teacher data: hundreds of thousands of groups x per-card teacher rollout ->
...
- Payoff: every future estimation run saves the branch-4->branch-8 cost multiplier
  (tens to thousandsx). The one-time cost amortizes across all future runs.
