...
- **MLP** 53->128->64->1, MSE regression. Offline test (unseen decks):
  ...
  | metric | value | note |
  | --- | --- | --- |
  | Spearman rho | **0.671** | ranks states moderately |
  | R^2 | 0.251 | explains 25% variance; overfits (val 0.43 -> test 0.25) |
...
seeds `best` at -inf so a complete line always wins (matches setup mode's play
propensity).
