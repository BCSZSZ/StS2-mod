...
internal static partial class Program
{
    ...
    private static int WriteGenerationPools(string[] args)
    {
        ...
        static bool InOtherHeroPool(SimulationCard card) =>
            InPool(card, "Ironclad")
            || InPool(card, "Silent")
            || InPool(card, "Defect")
            || InPool(card, "Necrobinder");
        ...
        [
            ...
            new("splash.otherHeroes.attack", card => InOtherHeroPool(card) && card.IsAttack, null,
                "Splash: Attacks from the other playable heroes' pools."),
        ];
        ...
        var output = new
        {
            ...
            note = "Generated by 'write-generation-pools'. 'pools' holds only faithfully simulatable cards "
                + "matching each generator's subject (Regent / current-hero / other-hero / Colorless). 'unsupportedPools' "
