...
| # | Fix | Cards | Status |
|---|---|---|---|
| 1 | Computed hit-count scaling (WithHitCount(CalculatedVar)) | LunarBlast, Radiate | ✅ done |
| 2 | `CardCmd.AutoPlay` loops | BeatDown, Catastrophe, DecisionsDecisions | ✅ done (earlier) |
| 3 | `RepeatVar(N)` | DecisionsDecisions, HeirloomHammer | ✅ done (earlier) |
| 4 | State-sourced draw-to-full | Scrawl | ✅ done |
| 5 | `BaseReplayCount +=` replay grant | HiddenGem | ✅ done |
| 6 | Power-effect → play-delta classification | Calamity, Entropy, PaleBlueDot, SpectrumShift, Stratagem, Tyranny, VoidForm | ✅ done |
| 7 | `PlayerCmd.EndTurn` downside | VoidForm | ✅ done |
| 8 | Self-return `CardPileCmd.Add(this,…)` | ShiningStrike, Bolas | ✅ done |
| 9 | Minor / cosmetic | Discovery/Splash/Jackpot/CosmicIndifference/Purity/SummonForth | ◻ assessed — see below |

## New classification (smoke run: 4 decks × 30 runs × turns 8; magnitudes indicative)

| Card | zh | Before | **After** | Smoke value/play (S / M) |
|---|---|---|---|---|
| LunarBlast | 月面射击 | source-credit, flat 4/4/4 | **source-credit** (now scales × skills played) | 9.9 / 9.8 |
| Radiate | 辐射 | source-credit, flat ~3.9 | **source-credit** (now scales × stars gained, AoE) | 5.9 / 4.7 |
...
- **Item 1 (conditional multi-hit):** added per-turn counters `SkillsPlayedThisTurn` /
  ...
  through the scaling channel (base × count) and zeroes the flat term (no double count). Confirmed
  game formula `hits = base + extra × multiplier` (LunarBlast = skills played; Radiate = stars gained,
...
- **Item 5 (replay grant):** parser emits `grantReplay` (amount = the `IntVar("Replay")` value) on
  ...
  `ReplayGrant × PlayValue(best draw-pile card)`. `grantReplay` is incomplete-attribution → play-delta.
