...
数值 = **play-delta（边际 dEV，每次直接打出的价值）**，来自 2026-07-02 全牌组重估。分 short(4 回合) / mid(8 回合) / long(14 回合) 三个时间跨度，按"未升级 mid"从高到低排序。负值属正常：前期铺垫/稀释成本，靠后回合或联动才回本。

> 数值只在同一口径内可比。以下 **主表 127 张** 都是 play-delta ??；文末 **10 张**（不可模拟的卡）仍沿用旧的静态 layer-17 估值，口径不同，**不可与主表直接比较**。另有 2 张全 0 卡未列出：Alchemize（炼制药水）、Royalties（版税）。

## 主表 . play-delta ??（127 张，按未升级 mid 降序）
